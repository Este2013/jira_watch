...
A Dart client for the **Jira Software (Agile) Cloud REST API**, generated from Atlassian's
OpenAPI specification — 105 operations.
...
```powershell
pwsh tool/generate_jira_api.ps1 -Package jira_software_api
```

That downloads the current spec to `spec/swagger.v3.json`, applies the fixups in
...
`spec/swagger.v3.json` is exactly what Atlassian publishes. The Dart generator needs three
