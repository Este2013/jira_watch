...
class UserDetails {
  ...
  @override
  String toString() => 'UserDetails[accountId=$accountId, accountType=$accountType, active=$active, avatarUrls=$avatarUrls, displayName=$displayName, emailAddress=$emailAddress, key=$key, name=$name, self=$self, timeZone=$timeZone]';
  ...
  static Map<String, List<UserDetails>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = UserDetails.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
