...
class JiraSettingsApi {
  JiraSettingsApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Response> getAdvancedSettingsWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/application-properties/advanced-settings';
  ...
  ///
  /// Returns the application properties that are accessible on the *Advanced Settings* page. To navigate to the *Advanced Settings* page in Jira, choose the Jira icon > **Jira settings** > **System**, **General Configuration** and then click **Advanced Settings** (in the upper right).  **[Permissions](#permissions) required:** *Administer Jira* [global permission](https://confluence.atlassian.com/x/x4dKLg).
  Future<List<ApplicationProperty>?> getAdvancedSettings({ Future<void>? abortTrigger, }) async {
    final response = await getAdvancedSettingsWithHttpInfo(abortTrigger: abortTrigger,);
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<ApplicationProperty>') as List)
        .cast<ApplicationProperty>()
  ...
  Future<Response> getApplicationPropertyWithHttpInfo({ String? key, String? permissionLevel, String? keyFilter, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/application-properties';
    ...
    if (permissionLevel != null) {
      queryParams.addAll(_queryParams('', 'permissionLevel', permissionLevel));
    }
    if (keyFilter != null) {
      queryParams.addAll(_queryParams('', 'keyFilter', keyFilter));
    }
  ...
  /// * [String] keyFilter:
  ///   When a `key` isn't provided, this filters the list of results by the application property `key` using a regular expression. For example, using `jira.lf.*` will return all application properties with keys that start with *jira.lf.*.
  Future<List<ApplicationProperty>?> getApplicationProperty({ String? key, String? permissionLevel, String? keyFilter, Future<void>? abortTrigger, }) async {
    final response = await getApplicationPropertyWithHttpInfo(key: key, permissionLevel: permissionLevel, keyFilter: keyFilter, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> getConfigurationWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/configuration';
  ...
  ///
  /// Returns the [global settings](https://confluence.atlassian.com/x/qYXKM) in Jira. These settings determine whether optional features (for example, subtasks, time tracking, and others) are enabled. If time tracking is enabled, this operation also returns the time tracking configuration.  **[Permissions](#permissions) required:** Permission to access Jira.
  Future<Configuration?> getConfiguration({ Future<void>? abortTrigger, }) async {
    final response = await getConfigurationWithHttpInfo(abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  ///
  /// * [SimpleApplicationPropertyBean] simpleApplicationPropertyBean (required):
  Future<Response> setApplicationPropertyWithHttpInfo(String id, SimpleApplicationPropertyBean simpleApplicationPropertyBean, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/application-properties/{id}'
      .replaceAll('{id}', id);
  ...
  ///
  /// * [SimpleApplicationPropertyBean] simpleApplicationPropertyBean (required):
  Future<ApplicationProperty?> setApplicationProperty(String id, SimpleApplicationPropertyBean simpleApplicationPropertyBean, { Future<void>? abortTrigger, }) async {
    final response = await setApplicationPropertyWithHttpInfo(id, simpleApplicationPropertyBean, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'ApplicationProperty',) as ApplicationProperty;
