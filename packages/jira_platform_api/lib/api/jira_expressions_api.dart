...
class JiraExpressionsApi {
  JiraExpressionsApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  /// Parameters:
  ///
  /// * [JiraExpressionForAnalysis] jiraExpressionForAnalysis (required):
  ...
  Future<Response> analyseExpressionWithHttpInfo(JiraExpressionForAnalysis jiraExpressionForAnalysis, { String? check, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/expression/analyse';
    ...
    if (check != null) {
      queryParams.addAll(_queryParams('', 'check', check));
    }
  ...
  /// * [String] check:
  ///   The check to perform:   *  `syntax` Each expression's syntax is checked to ensure the expression can be parsed. Also, syntactic limits are validated. For example, the expression's length.  *  `type` EXPERIMENTAL. Each expression is type checked and the final type of the expression inferred. Any type errors that would result in the expression failure at runtime are reported. For example, accessing properties that don't exist or passing the wrong number of arguments to functions. Also performs the syntax check.  *  `complexity` EXPERIMENTAL. Determines the formulae for how many [expensive operations](https://developer.atlassian.com/cloud/jira/platform/jira-expressions/#expensive-operations) each expression may execute.
  Future<JiraExpressionsAnalysis?> analyseExpression(JiraExpressionForAnalysis jiraExpressionForAnalysis, { String? check, Future<void>? abortTrigger, }) async {
    final response = await analyseExpressionWithHttpInfo(jiraExpressionForAnalysis, check: check, abortTrigger: abortTrigger,);
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'JiraExpressionsAnalysis',) as JiraExpressionsAnalysis;
  ...
  /// Parameters:
  ///
  /// * [JiraExpressionEvaluateRequestBean] jiraExpressionEvaluateRequestBean (required):
  ...
  Future<Response> evaluateJSISJiraExpressionWithHttpInfo(JiraExpressionEvaluateRequestBean jiraExpressionEvaluateRequestBean, { String? expand, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/expression/evaluate';

    // ignore: prefer_final_locals
    Object? postBody = jiraExpressionEvaluateRequestBean;
  ...
  /// * [String] expand:
  ///   Use [expand](#expansion) to include additional information in the response. This parameter accepts `meta.complexity` that returns information about the expression complexity. For example, the number of expensive operations used by the expression and how close the expression is to reaching the [complexity limit](https://developer.atlassian.com/cloud/jira/platform/jira-expressions/#restrictions). Useful when designing and debugging your expressions.
  Future<JExpEvaluateJiraExpressionResultBean?> evaluateJSISJiraExpression(JiraExpressionEvaluateRequestBean jiraExpressionEvaluateRequestBean, { String? expand, Future<void>? abortTrigger, }) async {
    final response = await evaluateJSISJiraExpressionWithHttpInfo(jiraExpressionEvaluateRequestBean, expand: expand, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'JExpEvaluateJiraExpressionResultBean',) as JExpEvaluateJiraExpressionResultBean;
  ...
  /// Parameters:
  ///
  /// * [JiraExpressionEvalRequestBean] jiraExpressionEvalRequestBean (required):
  ///   The Jira expression and the evaluation context.
  ///
  /// * [String] expand:
  ///   Use [expand](#expansion) to include additional information in the response. This parameter accepts `meta.complexity` that returns information about the expression complexity. For example, the number of expensive operations used by the expression and how close the expression is to reaching the [complexity limit](https://developer.atlassian.com/cloud/jira/platform/jira-expressions/#restrictions). Useful when designing and debugging your expressions.
  Future<Response> evaluateJiraExpressionWithHttpInfo(JiraExpressionEvalRequestBean jiraExpressionEvalRequestBean, { String? expand, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/rest/api/3/expression/eval';
  ...
  /// * [String] expand:
  ///   Use [expand](#expansion) to include additional information in the response. This parameter accepts `meta.complexity` that returns information about the expression complexity. For example, the number of expensive operations used by the expression and how close the expression is to reaching the [complexity limit](https://developer.atlassian.com/cloud/jira/platform/jira-expressions/#restrictions). Useful when designing and debugging your expressions.
  Future<JiraExpressionResult?> evaluateJiraExpression(JiraExpressionEvalRequestBean jiraExpressionEvalRequestBean, { String? expand, Future<void>? abortTrigger, }) async {
    final response = await evaluateJiraExpressionWithHttpInfo(jiraExpressionEvalRequestBean, expand: expand, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'JiraExpressionResult',) as JiraExpressionResult;
