...
class SmartLinkSingle {
  /// Returns a new [SmartLinkSingle] instance.
  SmartLinkSingle({
    this.id,
    this.type,
    this.status,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SmartLinkSingle &&
    other.id == id &&
    other.type == type &&
    other.status == status &&
    other.title == title &&
  ...
  @override
  String toString() => 'SmartLinkSingle[id=$id, type=$type, status=$status, title=$title, parentId=$parentId, parentType=$parentType, position=$position, authorId=$authorId, ownerId=$ownerId, createdAt=$createdAt, embedUrl=$embedUrl, spaceId=$spaceId, version=$version, links=$links]';
  ...
  /// Returns a new [SmartLinkSingle] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SmartLinkSingle? fromJson(dynamic value) {
    if (value is Map) {
      ...
      return SmartLinkSingle(
        ...
        links: SmartLinkLinks.fromJson(json[r'_links']),
      );
    }
    return null;
  }

  static List<SmartLinkSingle> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SmartLinkSingle>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SmartLinkSingle.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SmartLinkSingle> mapFromJson(dynamic json) {
    final map = <String, SmartLinkSingle>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SmartLinkSingle.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SmartLinkSingle-objects as value to a dart map
  static Map<String, List<SmartLinkSingle>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SmartLinkSingle>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SmartLinkSingle.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
