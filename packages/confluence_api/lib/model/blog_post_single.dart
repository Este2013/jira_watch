...
class BlogPostSingle {
  /// Returns a new [BlogPostSingle] instance.
  BlogPostSingle({
    this.id,
    this.status,
    this.title,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is BlogPostSingle &&
    other.id == id &&
    other.status == status &&
    other.title == title &&
  ...
  @override
  String toString() => 'BlogPostSingle[id=$id, status=$status, title=$title, spaceId=$spaceId, authorId=$authorId, createdAt=$createdAt, version=$version, body=$body, labels=$labels, properties=$properties, operations=$operations, likes=$likes, versions=$versions, isFavoritedByCurrentUser=$isFavoritedByCurrentUser, links=$links]';
  ...
  /// Returns a new [BlogPostSingle] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BlogPostSingle? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<BlogPostSingle> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BlogPostSingle>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BlogPostSingle.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BlogPostSingle> mapFromJson(dynamic json) {
    final map = <String, BlogPostSingle>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BlogPostSingle.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BlogPostSingle-objects as value to a dart map
  static Map<String, List<BlogPostSingle>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BlogPostSingle>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = BlogPostSingle.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
