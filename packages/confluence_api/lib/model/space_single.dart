...
class SpaceSingle {
  /// Returns a new [SpaceSingle] instance.
  SpaceSingle({
    this.id,
    this.key,
    this.name,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SpaceSingle &&
    other.id == id &&
    other.key == key &&
    other.name == name &&
    other.type == type &&
  ...
  @override
  String toString() => 'SpaceSingle[id=$id, key=$key, name=$name, type=$type, status=$status, authorId=$authorId, spaceOwnerId=$spaceOwnerId, createdAt=$createdAt, homepageId=$homepageId, description=$description, icon=$icon, labels=$labels, properties=$properties, operations=$operations, permissions=$permissions, links=$links]';
  ...
  /// Returns a new [SpaceSingle] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SpaceSingle? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<SpaceSingle> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SpaceSingle>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SpaceSingle.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SpaceSingle> mapFromJson(dynamic json) {
    final map = <String, SpaceSingle>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SpaceSingle.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SpaceSingle-objects as value to a dart map
  static Map<String, List<SpaceSingle>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SpaceSingle>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = SpaceSingle.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
