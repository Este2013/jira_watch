...
class UpdateTaskRequest {
  ...
  /// Status of the task.
  UpdateTaskRequestStatusEnum status;
  ...
  @override
  String toString() => 'UpdateTaskRequest[id=$id, localId=$localId, spaceId=$spaceId, pageId=$pageId, blogPostId=$blogPostId, status=$status, createdBy=$createdBy, assignedTo=$assignedTo, completedBy=$completedBy, createdAt=$createdAt, updatedAt=$updatedAt, dueAt=$dueAt, completedAt=$completedAt]';
  ...
  static UpdateTaskRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'status'), 'Required key "UpdateTaskRequest[status]" is missing from JSON.');
        assert(json[r'status'] != null, 'Required key "UpdateTaskRequest[status]" has a null value in JSON.');
        return true;
      }());
      ...
      return UpdateTaskRequest(
        ...
        status: UpdateTaskRequestStatusEnum.fromJson(json[r'status'])!,
...
/// Status of the task.
enum UpdateTaskRequestStatusEnum {
  complete._(r'complete'),
  incomplete._(r'incomplete'),
  ;

  /// Instantiate a new enum with the provided value.
  const UpdateTaskRequestStatusEnum._(this._value);
  ...
  /// Returns the instance of [UpdateTaskRequestStatusEnum] that was successfully decoded
  /// from the passed [value] on success, null otherwise.
  static UpdateTaskRequestStatusEnum? fromJson(dynamic value) => UpdateTaskRequestStatusEnumTypeTransformer().decode(value);

  /// Returns a [List] containing instances of [UpdateTaskRequestStatusEnum]
  /// that were successfully decoded from the passed [JSON][json].
  static List<UpdateTaskRequestStatusEnum> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UpdateTaskRequestStatusEnum>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UpdateTaskRequestStatusEnum.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
...
/// Transformation class that can [encode] an instance of [UpdateTaskRequestStatusEnum] to String,
/// and [decode] dynamic data back to [UpdateTaskRequestStatusEnum].
class UpdateTaskRequestStatusEnumTypeTransformer {
  factory UpdateTaskRequestStatusEnumTypeTransformer() => _instance ??= const UpdateTaskRequestStatusEnumTypeTransformer._();

  const UpdateTaskRequestStatusEnumTypeTransformer._();

  String encode(UpdateTaskRequestStatusEnum data) => data._value;
  ...
  UpdateTaskRequestStatusEnum? decode(dynamic data, {bool allowNull = true}) {
    if (data is UpdateTaskRequestStatusEnum) {
      return data;
    }
    if (data != null) {
      switch (data) {
        case r'complete': return UpdateTaskRequestStatusEnum.complete;
        case r'incomplete': return UpdateTaskRequestStatusEnum.incomplete;
        default:
          if (!allowNull) {
  ...
  static UpdateTaskRequestStatusEnumTypeTransformer? _instance;
}
