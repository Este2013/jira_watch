...
class PageBulk {
  /// Returns a new [PageBulk] instance.
  PageBulk({
    this.id,
    this.status,
    this.title,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PageBulk &&
    other.id == id &&
    other.status == status &&
    other.title == title &&
  ...
  @override
  String toString() => 'PageBulk[id=$id, status=$status, title=$title, spaceId=$spaceId, parentId=$parentId, parentType=$parentType, position=$position, authorId=$authorId, ownerId=$ownerId, lastOwnerId=$lastOwnerId, subtype=$subtype, createdAt=$createdAt, version=$version, body=$body, links=$links]';
  ...
  /// Returns a new [PageBulk] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PageBulk? fromJson(dynamic value) {
    if (value is Map) {
  ...
  static List<PageBulk> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PageBulk>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PageBulk.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PageBulk> mapFromJson(dynamic json) {
    final map = <String, PageBulk>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PageBulk.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PageBulk-objects as value to a dart map
  static Map<String, List<PageBulk>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PageBulk>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PageBulk.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
